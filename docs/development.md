...
- `web/`：Vite + React + TypeScript + Kumo standalone 样式，初始化状态页面、安全登录、受保护的消息工作区、同页重新登录及同源标签页退出通知。工作区支持多选追加、最多三个活动任务的页面内上传队列，以及附件下载。
...
状态查询仅返回 `uninitialized`、`initialized` 或 `storage_error`，禁止缓存且不泄露目录、账户或凭证。未初始化与存储异常不开放业务写入。文件多选队列、状态查询/停止/同次重试与附件下载已实现；同页重新登录先刷新有效限制、协调旧任务，再自动接续等待项，不自动重发失败项，服务器文件页现提供列表、下载和应用文件额度，服务器删除协议及 Web 双入口已实现，Android 及正式发布尚未实现；初始化成功不代表整个 Spec 001 已完成。GitHub 托管 CI 定义见 `.github/workflows/check.yml`，实际执行结果以对应提交的 Actions 检查为准。

### 服务器文件列表与应用内导航

消息工作区和服务器文件页使用同一会话层的草稿、发送状态及上传调度。切页只切换视图，前台且认证有效时等待项继续接续；刷新／关闭仍不恢复本地队列。文件页进入、返回前台及手动刷新读取最新一页并校正已加载文件状态，可加载更早文件；只展示已提交记录，不扫描磁盘。下载复用原有 HEAD 探测和浏览器附件 GET，不缓冲完整 Blob；文件页及文件消息提供共用确认的删除按钮。应用文件额度通过独立快照展示，进入、手动刷新及返回前台时更新；容量请求使用独立代次防止旧响应覆盖新快照，不以文件状态版本代替容量顺序。
...
Web 两个入口共用页面内删除上下文：请求未接受前显示请求处理中，接受后显示“删除处理中，空间尚未释放”；清理完成才从文件页移除，历史保留“服务器文件已删除”。未知结果只查询、不自动重发 DELETE，查询仍为可用也不恢复下载；用户再次确认才重试。FILE_IN_USE 结束本次流程，不排队删除。切页保留上下文，隐藏／认证失效暂停查询，同页重登恢复，主动退出清空。正常进度查询约每 2 秒，失败退避至最多 30 秒并遵守 Retry-After；容量独立查询，不乐观扣减。

复验使用 `web/tests/deletion.ts` 的双入口、跨页／跨会话和未知结果流程，以及 `web/tests/deletion-ordering.spec.ts` 的删除后分页与真实旧响应乱序流程。后者分别保留历史、发送结果、批量和单文件状态的旧响应，在较新删除完成状态到达后放行，验证不能恢复下载或重新插入列表。稳定版桌面 Chrome 人工复验仍须在已授权的隔离实例上确认两处指定文件及影响文案、保存副本不变、分页、清理反馈、容量更新和带草稿／队列的切页与认证恢复；记录实际验收提交及结果，不将自动化 Chromium 当作人工通过。
...
持久接受返回 202 及状态对象，已保存转待清理但总占用不变。重复删除中返回 202，已完成返回 200；未知标识返回 404。后台每轮最多处理 64 个删除，失败沿用 5–60 秒退避，轮转游标避免坏实体长期占住第一页。清理验证受管身份与可访问性，仅删除 UUID 对应实体，不跟随链接或递归删除异常目录；目录同步及条件事务完成后才释放额度。故障时保留删除中，重启继续。历史、发送标识和成功结果永久保留，重放返回当前状态，不重新上传。删除未知结果不得自动重发 DELETE，应查询状态。
...
选择前及选择后重新查询有效单文件限制；未知/离线不启动上传。多选及追加后校验并按选择顺序入队，等待项不申请服务器额度且可单独移除。本页依次请求准入，文件体最多三个并行；明确失败不阻塞后续项，重试由用户触发。结果未知时不自动重发、不创建新身份，暂停启动等待项；仍可追加选择。刷新后不恢复任务，应先检查消息历史。登录到期隐藏任务并保留本页引用，不主动截断已认证的文件体；同页重登先刷新有效限制，再按原发送身份协调所有活动或未知任务（包括此前查到仍活动的任务），确认后自动接续等待项。限制查询失败时保持暂停，可重查；失败项不自动重传，文本草稿不自动发送。主动退出及同源退出通知中止本页传输、尽力停止未提交尝试并清空引用，旧响应不能复活新认证周期的任务；服务器超时清理仍是兜底。文件发送可按发送标识查询服务器状态与已成功消息；页面可手动查询未知结果，在旧尝试安全终结后用新尝试身份从头重传，重试准备响应丢失仍复用已生成身份。页面提供按尝试停止：abort 只停止本页发数据，收到服务端裁决前仍显示未知；停止先于准备会登记无文件元数据的持久标记，后继准备须提供固定元数据。放弃确认保留页面协调记录并显示等待原因，不自动重发；登录后查询未确认任务，主动退出尽力停止未提交任务。未确认项在页面已登录、可见且网络在线时每 10 秒查询；网络恢复或返回前台时先协调未知项，再继续等待项。活动或未知项（包括放弃确认项）占页面名额；确认成功、失败或停止后释放名额，待清理残留仍占服务器额度。文本输入与发送不受文件队列阻塞。隔离 Caddy HTTPS 入口验证实际 PUT 路由持续 16 秒分块及附件路由认证（`bash tests/caddy_ingress.sh`）；浏览器验证真实后端的文件交换和响应丢失；`web/tests/file-session.ts`、`file-logout.ts` 覆盖同页到期恢复顺序、原队列续传、失败项及草稿不自动发送、同源退出与新认证周期的迟到响应隔离。`backend/tests/files.rs` 通过可控认证时间验证已通过鉴权的上传可在自然到期后完成，而新请求须重新登录（S002-F10/F11）。隔离入口与 Playwright Chromium 不代替真实 HTTPS、稳定版 Chrome 和长文件体的人工验收。可用 `FILEHOP_TEST_CHROME=1 bash tests/browser.sh` 在安装了稳定版 Chrome 的机器上验证真实浏览器下载（默认运行 Playwright Chromium）；本地回环及自签名隔离入口不代替正式环境验收。文件表与消息类型现合入尚未进入 `main` 的 `0001_next_release.sql`，从全新实例初始化并测试，不宣称提供从旧 `0001` 的升级路径。若开发实例已应用旧版 `0001`，其 checksum 已改变；普通 `serve` 不会升级它，也不得自动清理或重写 checksum。只有操作者确认实例可丢弃并核对数据库和文件目录后才能显式重建；需保留数据的实例须等待独立升级入口，不能用 `init` 代替。
