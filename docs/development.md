...
最终实体总量检查不代替传输中暂存峰值或失败残留额度验证。后者需结合 `backend/tests/files.rs`、`files_failure.rs`、`files_timeout.rs` 的真实存储、清理及预留边界，以及目标环境受控测量。隔离 Caddy 测试用每秒进展的上传和附件响应验证超过 15 秒仍可完成；它的上游是 HTTP 探针，不是完整应用，不能代替真实入口、应用 Cookie/Origin 或 Chrome + HTTPS 组合验收。
...
状态查询仅返回 `uninitialized`、`initialized` 或 `storage_error`，禁止缓存且不泄露目录、账户或凭证。未初始化与存储异常不开放业务写入。文件多选队列、状态查询/停止/同次重试与附件下载已实现；同页重新登录先刷新有效限制、协调旧任务，再自动接续等待项，不自动重发失败项，文件管理、Android 及正式发布尚未实现；初始化成功不代表整个 Spec 001 已完成。GitHub 托管 CI 定义见 `.github/workflows/check.yml`，实际执行结果以对应提交的 Actions 检查为准。
