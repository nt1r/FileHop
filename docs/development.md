...
历史游标独立于最近快照和本地发送；重新读取最近页不覆盖已建立的旧页边界。读取失败保留已有消息和边界，允许再次点击重试；最近页和旧页读取互斥，但不阻止独立发送。所有结果沿用同一消息合并与发送确认入口，认证失效或退出后的旧请求不能更新当前页面。这里不增加自动同步，也不改变最近页读取尚不能补齐多页新增的限制。
