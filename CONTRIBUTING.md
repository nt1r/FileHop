...
Workflows check PR branch policy and run application builds, static checks, real-storage tests, browser regression, shell syntax, and Compose configuration validation on GitHub-hosted runners. Strictly allowlisted documentation-only PRs into `dev` skip application checks but retain scope regression and branch policy checks. Development PRs additionally select container and HTTPS ingress checks by affected build inputs, configuration, and shared test orchestration; known independent browser fixtures do not trigger container builds. PRs into `main` and manual Application checks runs always include both environment checks; pushes to `dev`/`main` retain all base application checks. See the [development guide](docs/development.md#github-actions-检查分层) for selection details and limitations.
